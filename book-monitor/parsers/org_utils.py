...
def count_words(text: str) -> int:
    ...
    # Code blocks (everything between #+BEGIN_SRC and #+END_SRC)
    text = re.sub(r'#\+BEGIN_SRC.*?#\+END_SRC', '', text, flags=re.DOTALL | re.IGNORECASE)

    # Example blocks (everything between #+BEGIN_EXAMPLE and #+END_EXAMPLE)
    text = re.sub(r'#\+BEGIN_EXAMPLE.*?#\+END_EXAMPLE', '', text, flags=re.DOTALL | re.IGNORECASE)

    # Quote blocks (everything between #+BEGIN_QUOTE and #+END_QUOTE)
    text = re.sub(r'#\+BEGIN_QUOTE.*?#\+END_QUOTE', '', text, flags=re.DOTALL | re.IGNORECASE)

    # Remove any remaining non-block org directives (lines starting with #+)
...
def remove_org_markup(text: str) -> str:
    ...
    # Has to go *after* links translation or will break it
    text = re.sub(r'/([^/]+?)/', r'\1', text)
