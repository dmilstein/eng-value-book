...
Any stakeholder who has worked with engineers for any length of time knows that "tech debt" is a synonym for "terrible code".

When the team asks to deal with "tech debt", the stakeholder hears a request to give up something they actively want (the requested features), for something that has *no apparent value to them* (reducing the amount of "bad code" in the system).
...
They shouldn't have to prove themselves to the stakeholders -- aren't they the engineers? Shouldn't they be trusted to know what good code is?

Almost as if: by invoking "tech debt", they've *made an argument for value already*.

This is essentially the engineers fall back on their technical authority and invoking a *moral frame*: bad code is bad because it is bad, and getting rid of it is a good thing, and you have to just trust us.

Unfortunately, that kind of moral frame is a reliably terrible starting point for conversations with stakeholders.
...
Such conversations will leave both sides intensely frustrated.
...
But once again: the engineers aren't wrong -- there are important issues that need to be dealt with, that the stakeholders are either unaware of, or not properly weighting in their thinking.
