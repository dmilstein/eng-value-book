...
** A Story: The Quicksand In Which Alistair Is Sinking
...
# XXX possibly, trim the below

# The exec team held a series of vigorous debates about how to face this challenge. The Chief Marketing Officer made a case for getting into the home insurance market. The CEO, however, who has worked in insurance for decades, pushed back. "We know auto claims, our operations are built around auto claims, our *product* is built around auto claims. You could use the same *marketing* plan, but we'd need to create an entirely new *business* as the backend."
...
# XXX who wrote the pricing roadmap.

** The Core Challenge: Data, Why Did It Have To Be Data?!

Unfortunately, just about all of the new product opportunities will require some pretty extensive restructuring of their core product.

Which is... going to be, um, tricky.

The core data model was hastily assembled in the early, fervent days of AccuraClaim, and then just blindly appended to ever since. Newly-hired engineers shrink in horror when they discover that the central EZClaims table[fn:: The company was originally called EasyClaims, and no one is excited about doing an apocalyptic rewrite just to fix the table names] has not one, but *two* linked extension tables (EZClaimsExtra and EZClaimsExtra2), to glue vast amounts of extra metadata onto each claim.[fn:: Does one of those two extension tables contain a JSON blob column with even more data shoved opaquely into it, under an inconsistent set of keys? I guess we'll just have to find out!]
...
Our friend Alistair works on one of the teams that owns the core claim processing platform, both front- and back-end.

His team is trying to deliver a total overhaul of the claims review workflows. Under the hood, this will require breaking a bunch of key data out of EZClaims{Extra,Extra2}, and placing it into new, clean SQL tables (with sane names and the occasional use of an actual foreign key!).

On the product and marketing side, the new reports his team is building will form the basis for a premium product tier. Factoring out the underlying data will also allow other teams to create entirely new products to integrate AccurClaim's platform more deeply into the operations of their customers.

Or at least, that's all the plan.

Alistair also learns that things are not going great on that plan.

# exactly according to that plan

Changing the core data model of a product used to process hundreds of thousands of claims a day has turned out to be fraught with weird edge cases and risks. All the teams are months behind schedule. Every time they roll out a change to the claims review workflows, a horde of customers descend on them with angry pitchforks in hand.

And of course, Max Hartswell, who leads all of engineering in this domain, has somehow become the bottleneck for all decision-making.
