...
They could take some time to review their notes, maybe talk to an engineer to flush out a bit more detail and then say[fn:: Or write! In a memo or brief!] something like the following:
...
/As we've sprinted to build Forms 2.0, we've asked the teeam to get things out in the simplest form as quickly as possible. Which we're really glad about. But we think there's a very real chance that, if an ENT customer were to start using Forms 2.0 today, with their large data sets, they would experience painfully load times for almost all the key new pages. In fact, worst case, many those pages could just seem broken./

/We really don't want that to happen./

/Fortunately, we believe we have a couple of good options for speeding things up -- once we find the bottlenecks./
...
That approach to incremental development is the subject of our next chapter.
