...
An example I've seen more than once: building some hot new feature has the trivial prerequisite of restructuring the core data model for the business.
...
The second tactic, which can work as a nice complement to the user story approach, is to build a crude but usable prototype on top of the *current data model* (or, possibly, on top of "the simplest possible extension to that data model").
...
The core capability is that someone can step through various steps of that workflow, in order to understand what is / is not possible. (e.g. it's the kind of prototype where your engineers will likely end up spending a real chunk of time building tools to prep the *data* in various half-formed stages, so that you can demo the user story of "As a substitute teacher who has just been added to a class, I can review the outstanding assignments to students").

This is distinct from prototypes that allow you to explore a new user experience, but aren't hooked up to any actual code.

Such "visual" or "UX" prototypes can be *super* useful if you're trying to learn about user needs.

But if what you're trying to do is make visible the restrictions in your code and data, those experience prototypes can easily promise a version of the product that is forbiddingly hard to build.
...
Of *course* the CEO is eager to make *some* investment to prevent such problems in the future! Do you think the CEO doesn't care about the customer/the children??!
