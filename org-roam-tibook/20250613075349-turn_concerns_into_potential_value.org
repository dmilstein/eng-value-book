...
But each time, the team was able to find some way to get it going again, and the pains weren't that bad for customers. In fact, a *different* senior engineer, who has done some work as a DBA, believes that there's a lot more headroom in the current set up, if you could all just slow down for a minute to find it.

But then one team adds a feature that run a dozen expensive queries every time a customer blinks.

That team is being heaped with praise because the new feature is so damn sexy, so they have no time to talk as they rush to add something else which will wreak just utter havoc on your carefully crafted database indexing schemes.
...
# (though not as often as some enginers seem to think).
...
In particular, how would she think about the impact o  possible database problems on future profits.
...
She'll know that *plenty* of those companies got lucky, and found that with a few tweaks to their indexes plus moving expensive analytics queries to a read replica, they were fine for years (as that one DBA-turned-engineer suggests).
...
Given that it's unknown which world you're living in, Bertha will potential impact of database issues as a *weighted mix* of those two outcomes.

And the bad outcome, even if not particularly probable, is *very* bad indeed if it does hit. So the *risk* (likelihood x impact) genuinely depresses overall company value.

*** How to Make Risks Visible
...
In some fantasy world, a rational CEO would find a calibrated estimate of a 20% chance of the current DB architecture fully dead-ending to be *extremely* motivating.

Given just how bad it would be to dead-end, they'd be very much ready to invest engineering time in developing a more accurate estimate and some options.
...
Actual human beings, here on actual planet earth hear "a 20% risk" and think "Great, that's pretty unlikely" (no matter how bad things will be if the die turns up bad).
