...
** Some Supposedly Fun Ways To Estimate That I'll Never Do Again
...
** Commit to a Next Meaningful Decision Point
...
The fundamental definition of the first stage of this technical investment is: build a thing in no more than 3 weeks that will both create some incremental improvements *and* allow the stakeholder and the eng leader to make a collaborative decision about what to do next.

Notice how this helps them both of their key decisions -- how do we free up capacity in the short term, *and* do we need to change our medium-term goals?

For the former, the engineering leader is offering some creative problem-solving -- putting a key senior engineer on this technical investment will mean slowing planned work done, but they're brainstorming ways to make that okay. The product manager might change that plan, but by offering it, the eng leader is trying to jointly problem solve for the first decision.

For the second decision, around medium term expectations, the engineering leader sketches in some possible options (load testing, handling bottlenecks, etc), and makes clear a timeline on which they'll be able to update those options.

Also note how, because the milestone is designed around a "what do we do next?" decision, the engineer working on it has enormous flexibility to adjust scope, and hit the deadline.

If Andrea discovers that adding the SLI's is more work than expected, she can shrink her scope to the 1-2 very most important ones. You're then having exactly the conversation with the stakeholder you promised -- we have this initial visibility, we can now decide if it's sufficient, if we need more visibility, or if we see problems we want to immediate focus on.

It also prompts Andrea to, very usefully, ask you and the PM questions like:
