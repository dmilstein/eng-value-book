...
But also, you often want to also create visibility into the *value being created by the operational work*.
...
E.g. say your engineers spend some time every sprint helping set up tricky data import configurations for new customers.

Imagine you then go to the Important People at your company and say, "Can we create a dedicated Data Operations team so the engineers can spend less time on new customer onboarding?"
...
"It currently takes three weeks to onboard new customers, and delays in the back and forth to get data imports set up is the main driver. That work is currently being handled by the engineers, but a) they are expensive, and b) it often takes them a day or two for an engineer to find time to review a question from customers, which adds a lot of delays. We'd like to talk about finding a better home for that work, so we can both improve onboarding times and reduce costs. Our early estimate is that 80% of the work can be done by the more technical members of the help desk, if we can create more time for them."

That's speaking to potential benefits -- both better customer experiences, but also lower costs *for the operation itself* (as opposed to the argument of "But I want my engineers to work on the new shiny thing").
