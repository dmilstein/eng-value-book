...
Most engineering teams I've seen have, in addition to their "main" work, at least three buckets of things that take their time.
 ...
 - Resolving outages and incidents
...
First, whatever operational work the engineers are doing is presumably creating *some* form of value -- but often that value does not accrue to *their stakeholders*.
...
In so doing, they find out something terrifying -- all of their backups were in subtly wrong format, so the attempts to restore all stall out and then fail. The team quickly adjusts the format, takes a back up, verifies they can restore, and then breathes a huge sigh of relief (and likely inhales a big slug of whiskey).
...
However, even if their product team understands this (which is not typically the case), it's hard for them to be excited about it, when they are being beaten up daily by executives about the sluggish pace of delivery.
