...
First, Bertha understands that there might be value for the company if the engineers can reduce the amount of time they spend on operational work. Doing so could free up time for them to work on something potentially more valuable.

However, that's not the whole story.

Whatever operational work the engineers are doing is presumably creating *some* form of value. It would be a mistake to assume it is just waste.

Bertha care about that value, too.
...
However, even if their product team understands all if this (which is not always the case), it's hard for them to be excited about it... when they are being beaten up daily by executives about the sluggish pace of delivery.
