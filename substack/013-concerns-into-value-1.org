...
Now, you might well have read the above parable and thought: "Ugh, Dan, that was an obvious mistake by that team. Teams should always build an end-to-end solution, from day one, to avoid exactly that problem."

Unfortunately, that approach doesn't always work, either.
...
Consider the following approaches, each of which teams will sometimes consider as their next incremental chunk of work:
...
Let's see how each of those could or could *not* create value, depending on context:
 ...
 - *Build an interactive-but-fake protoype of the complete set of planned functionality*
   ...
   - The engineering team is confident in their ability to build that complete set of functionality

   - It's hard or impossible for customers to imagine how the product even *could* solve their problems

     Customers are near-universally terrible at *imagining* software that they can't see or interact with. If the team can allow them to interact with something where they can successfully imagine using it to solve their problems, the team can achieve one of two outcomes, *each of which* is valuable:
 ...
 - *Build lightweight versions of all the backend components, and wire them all together*
   ...
   - Those lightweight versions all add up to something that allows you to test with customers, via one of the increments above (and to do so in confidence that you're showing them something you can fully build)
   ...
   - It's going to take a while to build all those components, and it's not clear if customers want or will pay for a solution to the problem the system helps with.

   - You have a small team, and all the risks and questions are concentrated in one component
