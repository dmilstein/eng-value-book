...
The team does all the rituals, and does them as faithfully and well as they know how. Their sprints are a crisp two weeks long. They start each sprint on a Monday morning with a full team planning session. They collect around their board every day for standup, and then individual engineers pull well-groomed tickets. On the final Friday of the sprint, the engineers demo what they've built, and then the team runs a brief retro in the afternoon.
...
To learn more, let's rewind to the start of the project.

At the time, senior non-technical stakeholders grudgingly conceeded to the project being run "via Agile", because otherwise the engineers would have actually quit[fn:: More accurately, because the engineers and product team are so confidently dismissive of the way the stakeholders *want* to run the project. The tech team derisively calls the stakeholder's desired approach, "waterfall". The stakeholder doesn't really understand what the alternative to "waterfall" is, but they've been around software development for long enough to know "waterfall" is Very Bad Indeed, so they don't force the question]. But those stakeholders extracted a concession of their own from the product team: a commitment to complete the project by Q2 of the following year.
...
The engineers sat down and broke the work down into *increments*. They came up with *some* understanding of waht they would build first, what they'd build second,
